...
class PreRetrievalChunking:
    ...
    def create_and_store_chunks(self, pdf_directory: str, strategy: ChunkingStrategy) -> List[Document]:
        ...
        self.logger.info(f"🔧 Initializing Qdrant client and collection: {strategy.value}")
    ...
    def create_chunks_for_all_strategies(self, pdf_directory: str) -> None:
        """Create chunks for all strategies."""
        strategies = self.get_available_strategies()
        self.logger.info(f"🚀 Starting chunk creation for {len(strategies)} strategies")
        
        for strategy in strategies:
            self.create_and_store_chunks(pdf_directory, strategy)
            
        self.logger.info(f"🎉 Completed chunk creation for all {len(strategies)} strategies")


if __name__ == "__main__":
    chunking = PreRetrievalChunking()
    chunking.create_chunks_for_all_strategies(pdf_directory=str(config.SAMPLE_PAPERS_DIR))